--[[
    @Author Oogle
    Vindicta Hard Mode - Necromancy Script
    Version: 2.1
    ...
    Hard Mode Phases:
    - Phase 1 (22461): Vindicta alone - Soul Split
    - Phase 2 (22462): Gorvek + Vindicta - Soul Split  
    - Phase 3 (22463): Gorvek alone - DEFLECT MELEE REQUIRED
]]

local API = require("api")
local Timer = require("core.timer")
local Config = require("VindictaHM.config")
local Rotation = require("VindictaHM.rotation")

API.Write_fake_mouse_do(false)

local version = "2.1"
local scriptStartTime = os.time()
local lastBossSeenTime = os.time()  -- Initialize to now, not 0
local BOSS_GRACE_PERIOD = 3  -- Seconds to wait before declaring boss dead (phase transitions)
...
local Timers = {
    ...
    surgeToBoss = Timer.new({
        ...
        useTicks = true,
        condition = function()
            -- Find boss using ReadAllObjectsArray
            local allNpcs = API.ReadAllObjectsArray({1}, {-1}, {})
            if allNpcs then
                for _, npc in ipairs(allNpcs) do
                    local npcId = tonumber(npc.Id) or 0
                    for _, bossId in ipairs(Config.Boss.ids) do
                        if npcId == bossId then
                            local playerPos = API.PlayerCoord()
                            local bossPos = WPOINT.new(npc.Tile_XYZ.x, npc.Tile_XYZ.y, npc.Tile_XYZ.z)
        ...
        action = function()
            ...
            local bossPos = WPOINT.new(boss.Tile_XYZ.x, boss.Tile_XYZ.y, boss.Tile_XYZ.z)
...
local phase3PrayerActive = false
local lastPrayerActionTime = 0
local PRAYER_COOLDOWN = 2  -- Seconds between prayer actions

local function isPrayerActive(varbit)
    local vb = API.VB_FindPSettinOrder(varbit)
    return vb and vb.state == 1
end

local function canDoPrayerAction()
    return (os.time() - lastPrayerActionTime) >= PRAYER_COOLDOWN
end

local function activatePrayers()
    if prayersActivated then return end
    if not canDoPrayerAction() then return end
    
    log("Activating prayers (Soul Split + Ruination)...")
    ...
    phase3PrayerActive = false
    lastPrayerActionTime = os.time()
end

local function activatePhase3Prayers()
    -- Phase 3: Gorvek alone - MUST use Deflect Melee
    if phase3PrayerActive then return end
    if not canDoPrayerAction() then return end
    
    -- Check if Deflect Melee already active
    local deflectBuff = API.Buffbar_GetIDstatus(Config.Buffs.deflectMelee, false)
    if deflectBuff.found then
        phase3PrayerActive = true
        return
    end
    
    log("PHASE 3: Switching to Deflect Melee!")
    
    -- -- Turn off Soul Split first (use varbit check)
    -- if isPrayerActive(Config.Prayers.soulSplit.varbit) then
    --     API.DoAction_Ability(Config.Prayers.soulSplit.name, 1, API.OFF_ACT_GeneralInterface_route, true)
    --     API.RandomSleep2(100, 50, 50)
    -- end
    
    -- Turn on Deflect Melee
    API.DoAction_Ability(Config.Prayers.deflectMelee.name, 1, API.OFF_ACT_GeneralInterface_route, true)
    API.RandomSleep2(200, 200, 200)
    phase3PrayerActive = true
    lastPrayerActionTime = os.time()
end

local function ensurePhase3Prayer()
    -- Make sure Deflect Melee stays on during phase 3
    -- Only check every PRAYER_COOLDOWN seconds to avoid spam
    if not canDoPrayerAction() then return end
    
    -- Check buff ID for Deflect Melee
    local deflectBuff = API.Buffbar_GetIDstatus(Config.Buffs.deflectMelee, false)
    if not deflectBuff.found then
        log("Deflect Melee dropped! Re-activating...")
        API.DoAction_Ability(Config.Prayers.deflectMelee.name, 1, API.OFF_ACT_GeneralInterface_route, true)
        lastPrayerActionTime = os.time()
    end
end
...
local function getBoss()
    ...
    if allNpcs then
        for _, npc in ipairs(allNpcs) do
            ...
            for _, bossId in ipairs(Config.Boss.ids) do
                if npcId == bossId then
                    if not bossTypeLogged then
                        log("Boss found! ID: " .. npcId)
                        bossTypeLogged = true
                    end
                    return npc
                end
    ...
    -- Reset flag when boss not found (for next spawn)
    bossTypeLogged = false
    return nil
end
...
-- Check if player can perform actions (not in GCD/animation)
local function canAct()
    local anim = API.ReadPlayerAnim()
    -- -1 is idle, allow actions
    -- Also allow if animation is combat-related (some abilities have short anims)
    return anim == -1 or anim == 0
end
...
local function handleCombat()
    ...
    -- Track when we last saw the boss (for phase transition grace period)
    lastBossSeenTime = os.time()
    Player.state.inCombat = true
    
    -- Detect phase (Hard Mode: 3 phases) - use tonumber for safe comparison
    local bossId = tonumber(boss.Id) or 0
    if bossId == Config.Boss.phase1Id then
        Player.state.phase = 1  -- Vindicta alone
    elseif bossId == Config.Boss.phase2Id then
        Player.state.phase = 2  -- Gorvek + Vindicta
    elseif bossId == Config.Boss.phase3Id then
        Player.state.phase = 3  -- Gorvek alone
    end
    
    -- Prayer management based on phase (prayers can be toggled anytime)
    if Player.state.phase == 3 then
        activatePhase3Prayers()
        ensurePhase3Prayer()
    else
        activatePrayers()
    end
    
    -- Fire avoidance (highest priority - always check)
    Timers.fire:execute()

    -- Health management (always allowed)
    Timers.food:execute()
    Timers.prayerPotion:execute()
    
    -- Skip combat abilities if player is busy (prevents Input Disabled)
    if not canAct() then
        return true  -- Still in combat, just waiting
    end
...
local function update()
    ...
    elseif status == "FIGHTING" then
        if not handleCombat() then
            -- Boss not found - check grace period before declaring dead
            -- This prevents false "Boss defeated" during phase transitions
            local timeSinceBoss = os.time() - lastBossSeenTime
            local stillInCombat = API.LocalPlayer_IsInCombat_()
            
            if timeSinceBoss >= BOSS_GRACE_PERIOD and not stillInCombat then
                -- Boss truly dead - grace period passed and not in combat
                handlePostKill()
            else
                -- Likely phase transition - keep waiting
                log("Boss not found, waiting... (" .. timeSinceBoss .. "s, combat=" .. tostring(stillInCombat) .. ")")
            end
        end
